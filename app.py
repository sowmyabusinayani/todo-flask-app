...
def index():
    ...
    user_data = []
    for user in users:
        cur.execute("SELECT * FROM categories WHERE user_id = %s ORDER BY id", (user[0],))
        categories = cur.fetchall()
        category_data = []
        for cat in categories:
            cur.execute("SELECT * FROM tasks WHERE category_id = %s ORDER BY id", (cat[0],))
            tasks = cur.fetchall()
...
def add_category(user_id):
    ...
    if not name:
        return redirect('/')
    cur = conn.cursor()
    cur.execute("SELECT 1 FROM categories WHERE user_id = %s AND name = %s", (user_id, name))
    if cur.fetchone():
...
def add_task(category_id):
    ...
    if not name:
        return redirect('/')
    cur = conn.cursor()
    cur.execute("SELECT 1 FROM tasks WHERE category_id = %s AND name = %s", (category_id, name))
    if cur.fetchone():
        return redirect('/')
    cur.execute("INSERT INTO tasks (category_id, name) VALUES (%s, %s)", (category_id, name))
    conn.commit()
    cur.close()
    return redirect('/')
...
def edit_category(cat_id):
    ...
    if not new_name:
        return redirect('/')
    cur = conn.cursor()
    cur.execute("SELECT 1 FROM categories WHERE name = %s AND id != %s", (new_name, cat_id))
    if cur.fetchone():
        return redirect('/')
...
def edit_task(task_id):
    ...
    if not new_name:
        return redirect('/')
    cur = conn.cursor()
    cur.execute("SELECT 1 FROM tasks WHERE name = %s AND id != %s", (new_name, task_id))
    if cur.fetchone():
        return redirect('/')
    cur.execute("UPDATE tasks SET name = %s WHERE id = %s", (new_name, task_id))
    conn.commit()
    cur.close()
    return redirect('/')
...
def delete_user(user_id):
    cur = conn.cursor()
    cur.execute("DELETE FROM tasks WHERE category_id IN (SELECT id FROM categories WHERE user_id = %s)", (user_id,))
